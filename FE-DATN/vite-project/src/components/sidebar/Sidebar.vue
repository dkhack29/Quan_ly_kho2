...
function onKeydown(e) {
  if (e.key === 'Escape') {
    ...
    const emit = defineEmits(['switch-view', 'open-register', 'logout'])
...
<template>
  <!-- Fixed Header -->
  <nav class="fixed top-0 left-0 right-0 z-30 bg-white dark:bg-gray-300 h-20 shadow-sm">
          ...
          <div class="relative flex items-center ml-4">
            ...
            <transition name="fade">
              ...
              >
                      ...
                      <a @click="handleLogout" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Đăng xuất</a>
              </div>
            </transition>
          </div>
    </div>    
    ...
    >
      ...
      <nav class="py-2">
        <ul class="space-y-2 font-medium">
          <li><button @click="() => { emit('switch-view', 'home'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">🏠 Trang Chủ</button></li>
          ...
          <li><button @click="() => { emit('switch-view', 'tao-phieu-xuat'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Tạo Phiếu Xuất</button></li>
          <li><button @click="() => { emit('switch-view', 'tao-phieu-nhap'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Tạo Phiếu Nhập</button></li>
          ...
          <li><button @click="() => { emit('switch-view', 'nhan-vien'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Nhân Viên</button></li>
          <li><button @click="() => { emit('switch-view', 'tai-khoan'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Trang Tài Khoản</button></li>
          <li><button @click="() => { emit('switch-view', 'nha-kho'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Nhà Kho</button></li>
          <li><button @click="() => { emit('switch-view', 'thong-ke'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Thống Kê, Báo Cáo</button></li>
              <li><button @click="() => { emit('switch-view', 'xuat-nhap-file'); hideDrawer() }" class="w-full text-left flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-400">Xuất File, Nhập File</button></li>
              <li><button @click="handleLogout" class="w-full text-left flex items-center p-2 text-red-600 rounded-lg hover:bg-red-50">🚪 Đăng xuất</button></li>
            </ul>
