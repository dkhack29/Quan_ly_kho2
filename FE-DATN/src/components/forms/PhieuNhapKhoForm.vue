    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="showForm" class="bg-white rounded-lg shadow-sm border mb-8">
        <div class="px-6 py-4 border-b">
          <h2 class="text-lg font-semibold text-gray-900">{{ editingPhieu ? 'Chỉnh sửa phiếu nhập kho' : 'Tạo phiếu nhập kho mới' }}</h2>
        </div>
        ...
        <form @submit.prevent="savePhieu" class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Giá nhập -->
            <div>
              <label for="giaNhap" class="block text-sm font-medium text-gray-700 mb-2">
                Giá nhập (VNĐ) <span class="text-red-500">*</span>
              </label>
              <input
                type="number"
                id="giaNhap"
                v-model="currentPhieu.giaNhap"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
      ...
      <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b">
          <h2 class="text-lg font-semibold text-gray-900">Danh sách phiếu nhập kho</h2>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="phieu in paginatedPhieuNhapKho" :key="phieu.id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <span
                    :class="{
                      ...
                      'bg-blue-100 text-blue-800': phieu.tinhTrang === 'Đã nhập',
                    }"
                  >
                    {{ phieu.tinhTrang }}
                  </span>
          ...
          <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p class="text-sm text-gray-700">
                Hiển thị {{ (currentPage - 1) * itemsPerPage + 1 }} đến {{ Math.min(currentPage * itemsPerPage, filteredPhieuNhapKho.length) }} trong số {{ filteredPhieuNhapKho.length }} dữ liệu
              </p>
            </div>
            <div>
...
const phieuNhapKho = ref([
  {
    ...
    ghiChu: 'Phiếu nhập kho tháng 11'
  },
  {
    id: 2,
    maPhieu: 'PNK002',
    ...
    ghiChu: 'Nhập hàng từ nhà cung cấp mới'
  },
  {
    id: 3,
    maPhieu: 'PNK003',
  ...
  },
  {
    ...
    ghiChu: 'Nhập hàng theo đơn đặt hàng'
  }
])

// Computed
const filteredPhieuNhapKho = computed(() => {
  let filtered = phieuNhapKho.value
  ...
  filtered.sort((a, b) => {
    ...
    } else if (sortBy.value === 'giaNhap') {
      return b.giaNhap - a.giaNhap
    }
    return 0
  })
...
const totalPages = computed(() => Math.ceil(filteredPhieuNhapKho.value.length / itemsPerPage.value))

const paginatedPhieuNhapKho = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value
  const end = start + itemsPerPage.value
  return filteredPhieuNhapKho.value.slice(start, end)
})
...
// Methods
const savePhieu = () => {
  if (!currentPhieu.maPhieu || !currentPhieu.nguonNhap || !currentPhieu.tenHang || !currentPhieu.soLuong || !currentPhieu.giaNhap || !currentPhieu.tinhTrang || !currentPhieu.thoiGian) {
  ...
  if (editingPhieu.value) {
    ...
    const index = phieuNhapKho.value.findIndex(p => p.id === currentPhieu.id)
    if (index !== -1) {
      phieuNhapKho.value.splice(index, 1, { ...currentPhieu })
      alert('Cập nhật phiếu nhập kho thành công!')
    }
  } else {
    // Add new phieu
    const newId = phieuNhapKho.value.length ? Math.max(...phieuNhapKho.value.map(p => p.id)) + 1 : 1
    phieuNhapKho.value.push({ ...currentPhieu, id: newId })
